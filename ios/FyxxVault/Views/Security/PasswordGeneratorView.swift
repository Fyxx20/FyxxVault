    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if policy.mode == .random {
                Text(String(format: NSLocalizedString("generator.length %lld", comment: ""), policy.length)).font(FVFont.body(13)).foregroundStyle(.white.opacity(0.8))
                ...
                Stepper(String(format: NSLocalizedString("generator.word.count %lld", comment: ""), policy.wordsCount), value: $policy.wordsCount, in: 3...8).foregroundStyle(.white.opacity(0.85))
            }
